...
const API = import.meta.env.VITE_API_BASE_URL ||
  ...
  (typeof window !== "undefined" && window.location.hostname === "localhost"
    ? "http://localhost:3001"
    : "https://pulse-price-tracker-api.onrender.com");
...
function Sparkline({ history }) {
  ...
  return (
    <div>
      <svg className="chart" viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="Price history">
...
function ProductDetail({ selected, onBack }) {
  ...
  const loadDetail = () => {
    request(`/api/products/${selected.id}`)
      .then(setDetail)
      .catch(e => setError(e.message));
  };

  useEffect(loadDetail, [selected.id]);

  const handleScrapeNow = async () => {
    setScraping(true);
    setScrapeNotice("Scraping live store with Playwright...");
    try {
      await request(`/api/products/${selected.id}/scrape`, { method: "POST" });
      setScrapeNotice("Price successfully updated!");
      loadDetail();
    } catch (err) {
      setScrapeNotice(`Scrape failed: ${err.message}`);
    } finally {
      setScraping(false);
    }
  };
  ...
  return (
    <section className="detail">
          ...
          <tbody>
            {history.slice().reverse().map(row => (
              <tr key={row.id}>
                ...
                <td>{row.stock === 0 ? "Out of stock" : `${row.stock} left`}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="panel">
        ...
        <table>
          <thead>
            <tr><th>Time</th><th>Outcome</th><th>Detail</th></tr>
          </thead>
          <tbody>
            {logs.map(log => (
              <tr key={log.id}>
...
function App() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      request(`/api/catalog/search?q=${encodeURIComponent(query)}`)
        .then(setMatches)
  ...
  const handleCardScrape = async (e, product) => {
    e.stopPropagation();
    setScrapingIds(prev => new Set(prev).add(product.id));
    try {
      await request(`/api/products/${product.id}/scrape`, { method: "POST" });
      load();
    } catch (err) {
      setMessage(`Scrape error on ${product.name}: ${err.message}`);
    } finally {
      setScrapingIds(prev => {
    ...
    return (
      <main>
        <header>
          ...
          <p>Reliable price checks, with an honest audit trail.</p>
        </header>
        <ProductDetail selected={selected} onBack={() => { setSelected(null); load(); }} />
      </main>
    );
  }
  ...
  return (
    <main>
      ...
      <section className="hero">
        <div>
          <h1>Watch the price,<br/>not the page.</h1>
          <p>Track products from the INE demo store. Every attempt is recorded, including the ones that fail.</p>
        </div>
        <div className="search">
          <label htmlFor="search">Find a product to track</label>
          <input
            id="search"
            value={query}
            onChange={e => setQuery(e.target.value)}
            placeholder='Try "headphones" or a full product name'
          />
      ...
      <section>
        <div className="section-heading">
          ...
          <span>Scrapes run every 2 hours + on-demand</span>
        </div>
        <div className="cards">
          {products.map(product => {
            ...
            return (
              <div
                ...
                onKeyDown={e => { if (e.key === "Enter") setSelected(product); }}
              >
                <div className="card-top">
                  <span>{product.category}</span>
                  <span className={product.latest?.stock === 0 ? "out" : "in"}>
                    {product.latest
                      ? (product.latest.stock ? `${product.latest.stock} left` : "Out of stock")
                      : (isScraping ? "Scraping..." : "Pending")}
                  </span>
                </div>
                <h3>{product.name}</h3>
          ...
          {!products.length && (
            <div className="empty">Search the INE store above to choose the first product.</div>
          )}
        </div>
      </section>
    </main>
